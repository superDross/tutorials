...
class AuthorListViewTest(TestCase):
    ...
    def test_view_url_exists_at_desired_location(self):
        # get a response dict from our author page
    ...
    def test_view_uses_correct_template(self):
        ''' Check response contains the expected template file.'''
        response = self.client.get(reverse('authors'))
        self.assertEqual(response.status_code, 200)
    ...
    def test_pagination_is_ten(self):
        ...
        # response.context refers to context variables passed to the template
...
class LoanedBookInstancesByUserListViewTest(TestCase):
    ''' Testing a view that requires a LogIn to be viewable.'''
    ...
    def test_logged_in_uses_correct_template(self):
        ...
        self.assertTemplateUsed(
            response, 'bookinstance_list_borrowed_user.html')
...
class AuthorCreateTest(TestCase):
    def setUp(self):
        ...
        permission = Permission.objects.get(
            name="Can create, update and delete Author objects")
        test_user2.user_permissions.add(permission)
        test_user2.save()

        birth = datetime.date.today() - datetime.timedelta(days=10000)
        death = datetime.date.today() - datetime.timedelta(days=100)
        self.test_author = Author.objects.create(
            first_name='John', last_name='Smith',
            date_of_birth=birth, date_of_death=death
        )

    def test_permissions_dont_allow_create_access(self):
        login = self.client.login(
            username='testuser1', password='1X<ISRUkw+tuK')
        response = self.client.get(reverse('author_create'))
        self.assertEqual(response.status_code, 403)

    def test_permissions_allow_create_access(self):
        login = self.client.login(
            username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.get(reverse('author_create'))
        self.assertEqual(response.status_code, 200)
    ...
    def test_uses_correct_template(self):
        ...
        response = self.client.get(reverse('author_create'))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'generic_form.html')

    # Future Tests:
    #   - redirection to the new author page upon creation
    #   - form invalid if dob or dod in the future
    #   - 


class RenewBookInstancesViewTest(TestCase):
    ''' Testing a view that requires a permission to be viewable.'''
    ...
    def setUp(self):
        ...
        # ONLY test_user2 has the correct permissions.
